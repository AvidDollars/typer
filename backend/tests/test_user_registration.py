...
from typer.custom_exceptions import UserAlreadyRegisteredException
...
async def test_invalid_arguments_for_user_registration(async_client):
    ...
    response = await async_client.post("/register/", data=name_is_short)
    ...
    response = await async_client.post("/register/", data=name_has_invalid_symbol)
    assert response.status_code == 400

    # EMAIL ADDRESS IS NOT VALID
    email_is_invalid = dict(name="John", email="inv@lid", password="abc123DEF")
    response = await async_client.post("/register/", data=email_is_invalid)
    assert response.status_code == 400
